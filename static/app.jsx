...
console.log('SmritiSaathi build: 2026-09-14 offline-map-editor');
...
const offlineDb = window.Dexie ? new Dexie('smritisaathi_offline') : null;
if (offlineDb) offlineDb.version(1).stores({home_locations:'id, is_active', home_connections:'id, from_location_id, to_location_id', familiar_objects:'id, room_id, routine_id', assistance_events:'id, timestamp', sync_queue:'id, created_at'});
const storeForUrl = url => url.startsWith('/api/home/locations') ? 'home_locations' : url.startsWith('/api/home/connections') ? 'home_connections' : url.startsWith('/api/objects') ? 'familiar_objects' : url.startsWith('/api/assistance/events') ? 'assistance_events' : null;
const makeId = prefix => `${prefix}_${crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}_${Math.random().toString(36).slice(2)}`}`;
async function queueWrite(method, url, body) {
  const id = body.id || makeId('sync'); body.id = body.id || id;
  if (offlineDb && !(await offlineDb.sync_queue.get(id))) await offlineDb.sync_queue.put({id, method, url, body, created_at:new Date().toISOString(), retry_count:0});
  if ('serviceWorker' in navigator && 'SyncManager' in window) navigator.serviceWorker.ready.then(r => r.sync.register('smritisaathi-sync')).catch(()=>{});
  return id;
}
async function flushAssistanceQueue() {
  if (!offlineDb || !navigator.onLine) return;
  for (const item of await offlineDb.sync_queue.orderBy('created_at').toArray()) {
    try { const res = await apiFetch(item.url, {method:item.method, headers:{'Content-Type':'application/json'}, body:JSON.stringify(item.body)}); if (!res.ok) throw Error(); await offlineDb.sync_queue.delete(item.id); }
    catch (_) { await offlineDb.sync_queue.update(item.id, {retry_count:item.retry_count + 1}); break; }
  }
}
...
async function fetchWithCache(url, cacheKey) {
  ...
  if (offlineDb && store) {
    const local = await offlineDb[store].toArray();
    // Patient flows have a local, non-blocking path after their first successful load.
    if (local.length) { if (navigator.onLine) refreshCollection(url, store); return local; }
    try { return await refreshCollection(url, store); } catch (_) { return []; }
  }
  try {
...
async function refreshCollection(url, store) {
  const response = await apiFetch(url); if (!response.ok) throw Error('Unable to load');
  const data = await response.json(); if (offlineDb && store) await offlineDb[store].bulkPut(data);
  return data;
}
async function offlineWrite(method, url, body) {
  const store = storeForUrl(url); const item = {...body};
  if (!item.id && method === 'POST') item.id = makeId(url.includes('objects') ? 'obj' : url.includes('locations') ? 'loc' : url.includes('connections') ? 'conn' : 'evt');
  if (offlineDb && store && method !== 'DELETE') await offlineDb[store].put(item);
  if (offlineDb && store && method === 'DELETE') await offlineDb[store].delete(item.id);
  await queueWrite(method, url, item); await flushAssistanceQueue(); return item;
}
async function logAssistanceEvent(event) {
  return offlineWrite('POST', '/api/assistance/events', {...event, id:event.id || makeId('evt'), offline:!navigator.onLine});
}
window.addEventListener('online', flushAssistanceQueue);
document.addEventListener('visibilitychange', () => { if (!document.hidden) flushAssistanceQueue(); });
navigator.serviceWorker?.addEventListener('message', event => { if (event.data?.type === 'FLUSH_SYNC_QUEUE') flushAssistanceQueue(); });
...
class VoiceService {
  constructor() {
    ...
    this.enabled = true;
    // Optional deployment configuration: {url, apiKey, ttsPipeline, asrPipeline}.
    // Bhashini is only attempted when a caregiver/admin has supplied its current pipeline details.
    try { this.bhashini = JSON.parse(localStorage.getItem('smritisaathi_bhashini') || 'null'); } catch (_) { this.bhashini = null; }
    this.initVoices();
  }
  ...
  async listen(langId = 'hi') {
    const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!Recognition) return null;
    return new Promise(resolve => {
      const recognition = new Recognition(); recognition.lang = VOICE_PACKS[langId]?.code || 'en-IN'; recognition.interimResults = false; recognition.maxAlternatives = 1;
      recognition.onresult = e => resolve(e.results[0][0].transcript || ''); recognition.onerror = () => resolve(null); recognition.onend = () => resolve(null); recognition.start();
    });
  }
...
function localRoute(locations, connections, start, target) { const byId=Object.fromEntries(locations.map(x=>[x.id,x])); if(!byId[start]||!byId[target]) return null; const seen={[start]:null},q=[start]; for(let i=0;i<q.length;i++){for(const edge of connections){const next=edge.from_location_id===q[i]?edge.to_location_id:edge.to_location_id===q[i]?edge.from_location_id:null;if(next&&!Object.prototype.hasOwnProperty.call(seen,next)){seen[next]=[q[i],edge];q.push(next);}}} if(!Object.prototype.hasOwnProperty.call(seen,target)) return null; const route=[]; for(let at=target;at;at=seen[at]?.[0]) route.unshift({...byId[at],instruction:seen[at]?.[1]?.instruction||''}); return route; }
const ObjectRecognitionService = { model:null, async fingerprint(src){const image=new Image();image.src=src;await image.decode();const c=document.createElement('canvas');c.width=c.height=16;const x=c.getContext('2d');x.drawImage(image,0,0,16,16);return [...x.getImageData(0,0,16,16).data].filter((_,i)=>i%4===0);}, async recognize(image){const objects=await fetchWithCache('/api/objects','smritisaathi_cache_objects');const fp=await this.fingerprint(image);let best=null;for(const o of objects.filter(x=>x.image)){const known=await this.fingerprint(o.image);const score=1-known.reduce((n,v,i)=>n+Math.abs(v-fp[i]),0)/(255*fp.length);if(!best||score>best.score)best={o,score};}if(best&&best.score>.86)return {name:best.o.familiar_name||best.o.name,familiar_object_id:best.o.id,everyday_use:best.o.everyday_use,room_id:best.o.room_id,confidence:best.score};if(window.mobilenet){this.model=this.model||await mobilenet.load();const p=(await this.model.classify(image))[0];if(p&&p.probability>.55)return {name:p.className,familiar_object_id:null,everyday_use:'This is an everyday item. Please use it in the way that feels familiar to you.',confidence:p.probability};}return {name:'uncertain',familiar_object_id:null,confidence:0};} };
function LegacyWhereAmIView({setView,voiceLanguage,speechSpeed}) { const [rooms,setRooms]=useState([]),[room,setRoom]=useState(null),[voiceNote,setVoiceNote]=useState('');useEffect(()=>{fetchWithCache('/api/home/locations','smritisaathi_cache_locations').then(setRooms)},[]);const choose=x=>{setRoom(x);localStorage.setItem('smritisaathi_current_location',x.id);logAssistanceEvent({event_type:'WHERE_AM_I_USED',location_id:x.id});};const ask=async()=>{setVoiceNote('Listening…');const words=await voiceService.listen(voiceLanguage);const match=rooms.find(x=>words&&[x.name,x.familiar_name].filter(Boolean).some(n=>words.toLowerCase().includes(n.toLowerCase())));if(match)choose(match);else{setVoiceNote(words?'Please say the familiar room name.':'Microphone was not available.');voiceService.speak('Please say the familiar room name, or tap a room.',voiceLanguage,speechSpeed);}};return <div className="max-w-2xl mx-auto bg-white rounded-3xl p-8 text-center space-y-5">{room?<><div className="text-6xl">📍</div>{room.photo&&<img src={room.photo} className="w-40 h-28 object-cover rounded-2xl mx-auto"/>}<h2 className="font-heading font-bold text-3xl">{room.familiar_name||room.name}</h2><p>{room.description||`You are in ${room.name}.`}</p><SpeakerButton text={room.voice_description||room.description||`You are in ${room.familiar_name||room.name}.`} lang={voiceLanguage} speed={speechSpeed}/><button onClick={()=>setView('dashboard')} className="block mx-auto px-8 py-3 rounded-2xl bg-peach-500 text-white">OK</button></>:<><h2 className="font-heading font-bold text-2xl">Where am I?</h2><button onClick={ask} className="px-4 py-2 rounded-xl border">🎤 Say the room name</button>{voiceNote&&<p>{voiceNote}</p>}<div className="grid grid-cols-2 gap-3">{rooms.filter(x=>x.is_active).map(x=><button onClick={()=>choose(x)} className="p-5 rounded-2xl bg-cream-50 border font-semibold">📍 {x.familiar_name||x.name}</button>)}</div></>}</div>; }
function LegacyTakeMeThereView({setView,voiceLanguage,speechSpeed}) {const[rooms,setRooms]=useState([]),[connections,setConnections]=useState([]),[route,setRoute]=useState(null),[message,setMessage]=useState('');useEffect(()=>{Promise.all([fetchWithCache('/api/home/locations','smritisaathi_cache_locations'),fetchWithCache('/api/home/connections','smritisaathi_cache_connections')]).then(([a,b])=>{setRooms(a);setConnections(b)})},[]);const go=to=>{const from=localStorage.getItem('smritisaathi_current_location');if(!from){setMessage("Let's find your location first.");return;}const found=localRoute(rooms,connections,from,to.id);logAssistanceEvent({event_type:'ROUTE_REQUESTED',location_id:to.id,success:!!found});if(found)setRoute(found);else setMessage("I don't have a route to this place yet.");};return <div className="max-w-2xl mx-auto bg-white rounded-3xl p-8 space-y-5">{route?<><h2 className="font-heading font-bold text-2xl">Take me there</h2>{route.map((x,i)=><div className="p-4 rounded-2xl bg-cream-50"><b>{i+1}. {x.familiar_name||x.name}</b><p>{x.instruction}</p><SpeakerButton text={x.instruction||x.name} lang={voiceLanguage} speed={speechSpeed}/></div>)}<button onClick={()=>{logAssistanceEvent({event_type:'ROUTE_COMPLETED',location_id:route.at(-1)?.id});setView('dashboard')}} className="px-6 py-3 rounded-2xl bg-peach-500 text-white">DONE</button></>:<><h2 className="font-heading font-bold text-2xl">Where would you like to go?</h2>{message&&<p className="p-3 bg-amber-50 rounded-xl">{message}</p>}<div className="grid grid-cols-2 gap-3">{rooms.map(x=><button onClick={()=>go(x)} className="p-4 rounded-2xl border bg-cream-50">🏠 {x.familiar_name||x.name}</button>)}</div></>}</div>;}
function LegacyWhatIsThisView({setView,voiceLanguage,speechSpeed}) {const[result,setResult]=useState(null),[busy,setBusy]=useState(false);const recognize=async e=>{const f=e.target.files[0];if(!f)return;setBusy(true);const image=await new Promise(ok=>{const r=new FileReader();r.onload=()=>ok(r.result);r.readAsDataURL(f)});const j=await ObjectRecognitionService.recognize(image);setResult(j);setBusy(false);logAssistanceEvent({event_type:j.familiar_object_id?'OBJECT_RECOGNITION_SUCCESS':'OBJECT_RECOGNITION_UNCERTAIN',object_id:j.familiar_object_id,success:!!j.familiar_object_id,recognition_result:j.name,confidence:j.confidence});};return <div className="max-w-xl mx-auto bg-white rounded-3xl p-8 text-center space-y-5"><h2 className="font-heading font-bold text-2xl">What is this?</h2>{!result?<><button onClick={async()=>{const words=await voiceService.listen(voiceLanguage);if(words?.toLowerCase().includes('what'))voiceService.speak('Please take a photo of the object.',voiceLanguage,speechSpeed)}} className="px-4 py-2 rounded-xl border">🎤 Ask by voice</button><input type="file" accept="image/*" capture="environment" onChange={recognize} className="w-full p-4 rounded-xl border"/>{busy&&<p>Looking carefully…</p>}</>:result.name!=='uncertain'?<><h3 className="text-xl font-bold">{result.name}</h3><p>{result.everyday_use}</p><SpeakerButton text={`${result.name}. ${result.everyday_use||''}`} lang={voiceLanguage} speed={speechSpeed}/><button onClick={()=>setView('dashboard')} className="px-6 py-3 rounded-2xl bg-peach-500 text-white">DONE</button></>:<><p>I'm not sure about this object. Please try another photo.</p><button onClick={()=>setResult(null)} className="px-5 py-3 rounded-xl bg-peach-500 text-white">TRY AGAIN</button><button onClick={()=>setView('dashboard')} className="ml-2 px-5 py-3 rounded-xl border">ASK CAREGIVER</button></>}</div>;}

function WhatIsThisView({setView,voiceLanguage,speechSpeed}) { const [result,setResult]=useState(null),[busy,setBusy]=useState(false),fileRef=useRef(null); const recognize=async e=>{const file=e.target.files[0];if(!file)return;setBusy(true);try{const image=await fileAsDataUrl(file);const found=await ObjectRecognitionService.recognize(image);setResult(found);await logAssistanceEvent({event_type:found.familiar_object_id?'OBJECT_RECOGNITION_SUCCESS':found.name==='uncertain'?'OBJECT_RECOGNITION_UNCERTAIN':'OBJECT_RECOGNITION_GENERIC',object_id:found.familiar_object_id,success:found.name!=='uncertain',recognition_result:found.name});}finally{setBusy(false);}}; const ask=async()=>{const words=await voiceService.listen(voiceLanguage);if(words?.toLowerCase().includes('what'))fileRef.current?.click();}; return <div className="max-w-xl mx-auto bg-white rounded-3xl p-8 text-center space-y-5"><h2 className="font-heading font-bold text-2xl">What is this?</h2>{!result?<><button onClick={ask} className="px-4 py-2 rounded-xl border">🎤 Say “What is this?”</button><input ref={fileRef} type="file" accept="image/*" capture="environment" onChange={recognize} className="w-full p-4 rounded-xl border"/>{busy&&<p>Looking carefully…</p>}</>:result.familiar_object_id?<><p className="text-sage-700 font-semibold">A familiar object</p><h3 className="text-xl font-bold">{result.name}</h3><p>{result.everyday_use}</p><SpeakerButton text={`${result.name}. ${result.everyday_use||''}`} lang={voiceLanguage} speed={speechSpeed}/><button onClick={()=>setView('dashboard')} className="px-6 py-3 rounded-2xl bg-peach-500 text-white">DONE</button></>:result.name!=='uncertain'?<><p className="text-skysoft-700 font-semibold">I think this is</p><h3 className="text-xl font-bold">{result.name}</h3><p>{result.everyday_use}</p><SpeakerButton text={`${result.name}. ${result.everyday_use}`} lang={voiceLanguage} speed={speechSpeed}/><button onClick={()=>setView('dashboard')} className="px-6 py-3 rounded-2xl bg-peach-500 text-white">DONE</button></>:<><p>I'm not sure about this object. Please try another photo.</p><button onClick={()=>setResult(null)} className="px-5 py-3 rounded-xl bg-peach-500 text-white">TRY AGAIN</button><button onClick={()=>setView('dashboard')} className="ml-2 px-5 py-3 rounded-xl border">ASK CAREGIVER</button></>}</div>; }
function LegacyHomeManager() {
  const [locations, setLocations] = useState([]), [connections, setConnections] = useState([]), [name, setName] = useState(''), [familiar, setFamiliar] = useState(''), [from, setFrom] = useState(''), [to, setTo] = useState(''), [instruction, setInstruction] = useState(''), [direction,setDirection]=useState(''), [description,setDescription]=useState(''), [voiceDescription,setVoiceDescription]=useState(''), [photo,setPhoto]=useState('');
  ...
  useEffect(() => { load(); }, []);
  const addLocation = async e => { e.preventDefault(); await offlineWrite('POST','/api/home/locations',{name,familiar_name:familiar || name,description,voice_description:voiceDescription,photo,is_active:true}); setName('');setFamiliar('');setDescription('');setVoiceDescription('');setPhoto('');load(); };
  const addConnection = async e => { e.preventDefault(); await offlineWrite('POST','/api/home/connections',{from_location_id:from,to_location_id:to,instruction,direction}); setInstruction('');setDirection('');load(); };
...
function LegacyObjectsManager() {
  const [objects,setObjects]=useState([]),[locations,setLocations]=useState([]),[name,setName]=useState(''),[use,setUse]=useState(''),[room,setRoom]=useState(''),[image,setImage]=useState(''),[voice,setVoice]=useState('');
  ...
  const add=async e=>{e.preventDefault();await offlineWrite('POST','/api/objects',{name,everyday_use:use,room_id:room,image,voice_explanation:voice});setName('');setUse('');setImage('');setVoice('');load();};
...
async function fileAsDataUrl(file) { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload=()=>resolve(reader.result); reader.onerror=reject; reader.readAsDataURL(file); }); }
function HomeMapDiagram({locations,connections}) { const points=locations.map((x,i)=>({...x,x:70+(i%3)*180,y:55+Math.floor(i/3)*110})); const byId=Object.fromEntries(points.map(x=>[x.id,x])); return <svg viewBox="0 0 600 300" className="w-full min-h-[180px] bg-cream-50 rounded-2xl border">{connections.map(c=>byId[c.from_location_id]&&byId[c.to_location_id]&&<line key={c.id} x1={byId[c.from_location_id].x} y1={byId[c.from_location_id].y} x2={byId[c.to_location_id].x} y2={byId[c.to_location_id].y} stroke="#5A9B72" strokeWidth="3"/>)}{points.map(p=><g key={p.id}><circle cx={p.x} cy={p.y} r="34" fill="#FEEFE6" stroke="#E26334" strokeWidth="2"/><text x={p.x} y={p.y+5} textAnchor="middle" fontSize="12" fill="#2D3748">{(p.familiar_name||p.name).slice(0,12)}</text></g>)}</svg>; }
function LegacyHomeManagerV1() {
  const blank={name:'',familiar_name:'',description:'',voice_description:'',photo:'',is_active:true};
  const [locations,setLocations]=useState([]),[connections,setConnections]=useState([]),[location,setLocation]=useState(blank),[editingLocation,setEditingLocation]=useState(null),[connection,setConnection]=useState({from_location_id:'',to_location_id:'',instruction:'',voice_instruction:'',direction:'',distance_estimate:''}),[editingConnection,setEditingConnection]=useState(null);
  const load=async()=>{const [a,b]=await Promise.all([fetchWithCache('/api/home/locations','smritisaathi_cache_locations'),fetchWithCache('/api/home/connections','smritisaathi_cache_connections')]);setLocations(a);setConnections(b);}; useEffect(()=>{load();},[]);
  const saveLocation=async e=>{e.preventDefault();const item={...location,id:editingLocation?.id};await offlineWrite(editingLocation?'PUT':'POST',editingLocation?`/api/home/locations/${editingLocation.id}`:'/api/home/locations',item);setLocation(blank);setEditingLocation(null);await load();};
  const saveConnection=async e=>{e.preventDefault();const item={...connection,id:editingConnection?.id};await offlineWrite(editingConnection?'PUT':'POST',editingConnection?`/api/home/connections/${editingConnection.id}`:'/api/home/connections',item);setConnection({from_location_id:'',to_location_id:'',instruction:'',voice_instruction:'',direction:'',distance_estimate:''});setEditingConnection(null);await load();};
  const remove=async(type,item)=>{if(!window.confirm(`Delete this ${type}?`))return;const url=type==='location'?`/api/home/locations/${item.id}`:`/api/home/connections/${item.id}`;await offlineWrite('DELETE',url,{id:item.id});await load();};
  return <div className="space-y-6 animate-fadeIn"><div><h2 className="font-heading font-bold text-2xl">Home Assistance</h2><p className="text-sm text-stone-500">Add rooms and the familiar paths between them.</p></div><div className="grid lg:grid-cols-2 gap-6"><form onSubmit={saveLocation} className="bg-white rounded-3xl p-6 border space-y-3"><h3 className="font-bold">{editingLocation?'Edit location':'Add location'}</h3><input required value={location.name} onChange={e=>setLocation({...location,name:e.target.value})} placeholder="Room name" className="w-full p-3 rounded-xl border"/><input value={location.familiar_name} onChange={e=>setLocation({...location,familiar_name:e.target.value})} placeholder="Familiar name" className="w-full p-3 rounded-xl border"/><textarea value={location.description} onChange={e=>setLocation({...location,description:e.target.value})} placeholder="Description" className="w-full p-3 rounded-xl border"/><textarea value={location.voice_description} onChange={e=>setLocation({...location,voice_description:e.target.value})} placeholder="Voice description" className="w-full p-3 rounded-xl border"/><input type="file" accept="image/*" onChange={async e=>e.target.files[0]&&setLocation({...location,photo:await fileAsDataUrl(e.target.files[0])})}/>{location.photo&&<img src={location.photo} className="w-20 h-16 object-cover rounded-lg"/>}<button className="px-4 py-2 rounded-xl bg-sage-600 text-white">Save location</button>{editingLocation&&<button type="button" onClick={()=>{setEditingLocation(null);setLocation(blank)}} className="ml-2 px-4 py-2 rounded-xl border">Cancel</button>}</form><form onSubmit={saveConnection} className="bg-white rounded-3xl p-6 border space-y-3"><h3 className="font-bold">{editingConnection?'Edit connection':'Connect locations'}</h3><select required value={connection.from_location_id} onChange={e=>setConnection({...connection,from_location_id:e.target.value})} className="w-full p-3 rounded-xl border"><option value="">From…</option>{locations.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><select required value={connection.to_location_id} onChange={e=>setConnection({...connection,to_location_id:e.target.value})} className="w-full p-3 rounded-xl border"><option value="">To…</option>{locations.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><input required value={connection.instruction} onChange={e=>setConnection({...connection,instruction:e.target.value})} placeholder="Walk straight to the kitchen" className="w-full p-3 rounded-xl border"/><input value={connection.direction} onChange={e=>setConnection({...connection,direction:e.target.value})} placeholder="Direction, e.g. turn left" className="w-full p-3 rounded-xl border"/><input value={connection.voice_instruction} onChange={e=>setConnection({...connection,voice_instruction:e.target.value})} placeholder="Voice instruction" className="w-full p-3 rounded-xl border"/><button className="px-4 py-2 rounded-xl bg-sage-600 text-white">Save connection</button></form></div><div className="bg-white rounded-3xl p-6 border"><h3 className="font-bold mb-3">Connected home map</h3><HomeMapDiagram locations={locations} connections={connections}/><div className="grid md:grid-cols-2 gap-3 mt-4">{locations.map(x=><div key={x.id} className="p-3 border rounded-xl flex gap-3 items-center">{x.photo&&<img src={x.photo} className="w-12 h-12 object-cover rounded"/>}<span className="flex-1"><b>{x.familiar_name||x.name}</b><small className="block">{x.description}</small></span><button onClick={()=>{setEditingLocation(x);setLocation(x)}}>Edit</button><button onClick={()=>remove('location',x)}>Delete</button></div>)}</div><div className="mt-4 space-y-2">{connections.map(c=><div key={c.id} className="p-3 border rounded-xl">{locations.find(x=>x.id===c.from_location_id)?.name} → {locations.find(x=>x.id===c.to_location_id)?.name}: {c.instruction} <button className="ml-2" onClick={()=>{setEditingConnection(c);setConnection(c)}}>Edit</button><button className="ml-2" onClick={()=>remove('connection',c)}>Delete</button></div>)}</div></div></div>;
}
function ObjectsManager() {
  const blank={name:'',familiar_name:'',everyday_use:'',room_id:'',person_id:'',routine_id:'',voice_explanation:'',image:''}; const [objects,setObjects]=useState([]),[locations,setLocations]=useState([]),[family,setFamily]=useState([]),[routines,setRoutines]=useState([]),[form,setForm]=useState(blank),[editing,setEditing]=useState(null);
  const load=async()=>{const [a,b,c,d]=await Promise.all([fetchWithCache('/api/objects','smritisaathi_cache_objects'),fetchWithCache('/api/home/locations','smritisaathi_cache_locations'),apiFetch('/api/family').then(r=>r.json()),apiFetch('/api/routines').then(r=>r.json())]);setObjects(a);setLocations(b);setFamily(c);setRoutines(d);};useEffect(()=>{load();},[]);
  const save=async e=>{e.preventDefault();const item={...form,id:editing?.id};await offlineWrite(editing?'PUT':'POST',editing?`/api/objects/${editing.id}`:'/api/objects',item);setForm(blank);setEditing(null);await load();}; const remove=async x=>{if(window.confirm('Delete this familiar object?')){await offlineWrite('DELETE',`/api/objects/${x.id}`,{id:x.id});await load();}};
  return <div className="space-y-6 animate-fadeIn"><div><h2 className="font-heading font-bold text-2xl">Familiar Objects</h2><p className="text-sm text-stone-500">Link personal items to familiar people, rooms, and routines.</p></div><form onSubmit={save} className="bg-white rounded-3xl p-6 border grid md:grid-cols-2 gap-3"><input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Object name" className="p-3 rounded-xl border"/><input value={form.familiar_name} onChange={e=>setForm({...form,familiar_name:e.target.value})} placeholder="Familiar name" className="p-3 rounded-xl border"/><input value={form.everyday_use} onChange={e=>setForm({...form,everyday_use:e.target.value})} placeholder="Everyday use" className="p-3 rounded-xl border"/><select value={form.room_id} onChange={e=>setForm({...form,room_id:e.target.value})} className="p-3 rounded-xl border"><option value="">Associated room</option>{locations.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><select value={form.person_id} onChange={e=>setForm({...form,person_id:e.target.value})} className="p-3 rounded-xl border"><option value="">Associated person</option>{family.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><select value={form.routine_id} onChange={e=>setForm({...form,routine_id:e.target.value})} className="p-3 rounded-xl border"><option value="">Associated routine</option>{routines.map(x=><option key={x.id} value={x.id}>{x.category||x.name}</option>)}</select><textarea value={form.voice_explanation} onChange={e=>setForm({...form,voice_explanation:e.target.value})} placeholder="Voice explanation" className="p-3 rounded-xl border"/><input type="file" accept="image/*" onChange={async e=>e.target.files[0]&&setForm({...form,image:await fileAsDataUrl(e.target.files[0])})}/>{form.image&&<img src={form.image} className="w-20 h-16 object-cover rounded"/>}<div><button className="px-4 py-2 rounded-xl bg-sage-600 text-white">Save object</button>{editing&&<button type="button" onClick={()=>{setEditing(null);setForm(blank)}} className="ml-2 px-4 py-2 rounded-xl border">Cancel</button>}</div></form><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{objects.map(x=><div key={x.id} className="bg-white rounded-2xl p-5 border">{x.image&&<img src={x.image} className="w-full h-28 object-cover rounded-xl mb-3"/>}<h3 className="font-bold">{x.familiar_name||x.name}</h3><p className="text-sm">{x.everyday_use}</p><button onClick={()=>{setEditing(x);setForm(x)}} className="mt-3">Edit</button><button onClick={()=>remove(x)} className="ml-3">Delete</button></div>)}</div></div>;
}
const RoomVisionService={async find(image){const rooms=await fetchWithCache('/api/home/locations','smritisaathi_cache_locations');const query=await ObjectRecognitionService.fingerprint(image);let best=null;for(const room of rooms){for(const ref of [room.photo,...(room.reference_images||[])].filter(Boolean)){try{const known=await ObjectRecognitionService.fingerprint(ref);const score=1-known.reduce((sum,v,i)=>sum+Math.abs(v-query[i]),0)/(255*query.length);if(!best||score>best.score)best={room,score};}catch(_){}}}return best&&best.score>=.84?best.room:null;}};
function WhereAmIView({setView,voiceLanguage,speechSpeed}){const[result,setResult]=useState(null),[message,setMessage]=useState(''),[busy,setBusy]=useState(false);const scan=async e=>{const file=e.target.files[0];if(!file)return;setBusy(true);setMessage('Looking at the room…');try{const room=await RoomVisionService.find(await fileAsDataUrl(file));if(room){setResult(room);localStorage.setItem('smritisaathi_current_location',room.id);await logAssistanceEvent({event_type:'WHERE_AM_I_USED',location_id:room.id,success:true,recognition_result:'room_camera_match'});voiceService.speak(room.voice_description||`You are in ${room.familiar_name||room.name}.`,voiceLanguage,speechSpeed);}else setMessage("I'm not sure which part of the home this is. Please try another view.");}finally{setBusy(false);}};return <div className="max-w-xl mx-auto bg-white rounded-3xl p-8 text-center space-y-5"><div className="text-6xl">📷</div><h2 className="font-heading font-bold text-2xl">Where am I?</h2>{result?<><h3 className="text-2xl font-bold">You are in {result.familiar_name||result.name}</h3>{result.photo&&<img src={result.photo} className="w-48 h-32 object-cover rounded-2xl mx-auto"/>}<p>{result.description}</p><SpeakerButton text={result.voice_description||`You are in ${result.familiar_name||result.name}.`} lang={voiceLanguage} speed={speechSpeed}/><button onClick={()=>setView('dashboard')} className="px-7 py-3 rounded-2xl bg-peach-500 text-white">OK</button></>:<><p>Point the camera at the room.</p><input type="file" accept="image/*" capture="environment" onChange={scan} className="w-full p-4 rounded-xl border"/>{busy&&<p>Looking carefully…</p>}{message&&!busy&&<p className="text-stone-600">{message}</p>}</>}</div>;}
function TakeMeThereView({setView,voiceLanguage,speechSpeed}){const[rooms,setRooms]=useState([]),[connections,setConnections]=useState([]),[route,setRoute]=useState(null),[message,setMessage]=useState('');useEffect(()=>{Promise.all([fetchWithCache('/api/home/locations','smritisaathi_cache_locations'),fetchWithCache('/api/home/connections','smritisaathi_cache_connections')]).then(([a,b])=>{setRooms(a);setConnections(b);});},[]);const start=to=>{const from=localStorage.getItem('smritisaathi_current_location'),found=localRoute(rooms,connections,from,to.id);if(!found){setMessage(from?"I don't have a route to this place yet.":'Use Where am I? first so I can find your starting place.');return;}setRoute(found);logAssistanceEvent({event_type:'ROUTE_REQUESTED',location_id:to.id,success:true});};if(route)return <div className="max-w-xl mx-auto bg-white rounded-3xl p-8 text-center space-y-4"><h2 className="font-heading font-bold text-2xl">Follow the red dots</h2><div className="relative overflow-hidden rounded-2xl bg-stone-900 h-72"><video autoPlay playsInline className="w-full h-full object-cover" ref={v=>{if(v&&navigator.mediaDevices?.getUserMedia&&!v.srcObject)navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}}).then(s=>v.srcObject=s).catch(()=>{});}}/><div className="absolute inset-0 flex items-center justify-center gap-5 pointer-events-none"><i className="w-7 h-7 rounded-full bg-red-500 animate-pulse"/><i className="w-7 h-7 rounded-full bg-red-500 animate-pulse"/><i className="w-7 h-7 rounded-full bg-red-500 animate-pulse"/></div></div><p className="font-semibold">{route[1]?.instruction||'You have reached the destination.'}</p><SpeakerButton text={route[1]?.instruction||'You have reached the destination.'} lang={voiceLanguage} speed={speechSpeed}/><button onClick={()=>{logAssistanceEvent({event_type:'ROUTE_COMPLETED',location_id:route.at(-1)?.id});setView('dashboard');}} className="px-6 py-3 rounded-2xl bg-peach-500 text-white">DONE</button></div>;return <div className="max-w-xl mx-auto bg-white rounded-3xl p-8 space-y-5"><h2 className="font-heading font-bold text-2xl">Where would you like to go?</h2>{message&&<p>{message}</p>}<div className="grid grid-cols-2 gap-3">{rooms.map(r=><button key={r.id} onClick={()=>start(r)} className="p-4 rounded-2xl border bg-cream-50">{r.familiar_name||r.name}</button>)}</div></div>;}
function LegacyHomeManagerV2(){const blank={name:'',familiar_name:'',description:'',voice_description:'',photo:'',reference_images:[],is_active:true};const[rooms,setRooms]=useState([]),[form,setForm]=useState(blank),[editing,setEditing]=useState(null);const load=()=>fetchWithCache('/api/home/locations','smritisaathi_cache_locations').then(setRooms);useEffect(()=>{load();},[]);const addPhotos=async files=>{const images=await Promise.all([...files].map(fileAsDataUrl));setForm(x=>({...x,reference_images:[...(x.reference_images||[]),...images],photo:x.photo||''}));};const save=async e=>{e.preventDefault();await offlineWrite(editing?'PUT':'POST',editing?`/api/home/locations/${editing.id}`:'/api/home/locations',{...form,id:editing?.id,photo:form.reference_images?.[0]||form.photo});setForm(blank);setEditing(null);load();};return <div className="space-y-5"><h2 className="font-heading font-bold text-2xl">Home Camera Map</h2><p className="text-sm">Add 3–6 photos from each room corner. The patient camera compares a live view to these references.</p><form onSubmit={save} className="bg-white rounded-3xl p-6 border space-y-3"><input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Room name" className="w-full p-3 border rounded-xl"/><input value={form.familiar_name} onChange={e=>setForm({...form,familiar_name:e.target.value})} placeholder="Familiar name" className="w-full p-3 border rounded-xl"/><textarea value={form.voice_description} onChange={e=>setForm({...form,voice_description:e.target.value})} placeholder="What should be spoken?" className="w-full p-3 border rounded-xl"/><input type="file" accept="image/*" multiple onChange={e=>addPhotos(e.target.files)}/><p className="text-sm">{form.reference_images?.length||0} reference photos selected</p><button className="px-4 py-2 rounded-xl bg-sage-600 text-white">{editing?'Update room':'Save room photos'}</button></form><div className="grid md:grid-cols-2 gap-3">{rooms.map(r=><div key={r.id} className="bg-white border rounded-xl p-4"><b>{r.familiar_name||r.name}</b><p className="text-sm">{(r.reference_images||[]).length|| (r.photo?1:0)} reference photo(s)</p><button onClick={()=>{setEditing(r);setForm({...blank,...r,reference_images:r.reference_images||[r.photo].filter(Boolean)})}}>Edit photos</button></div>)}</div></div>;}
function HomeManager(){const blank={name:'',familiar_name:'',description:'',voice_description:'',reference_images:[],is_active:true};const[rooms,setRooms]=useState([]),[edges,setEdges]=useState([]),[form,setForm]=useState(blank),[edge,setEdge]=useState({from_location_id:'',to_location_id:'',instruction:''}),[editing,setEditing]=useState(null);const load=async()=>{const[a,b]=await Promise.all([fetchWithCache('/api/home/locations','smritisaathi_cache_locations'),fetchWithCache('/api/home/connections','smritisaathi_cache_connections')]);setRooms(a);setEdges(b);};useEffect(()=>{load();},[]);const photos=async files=>{const images=await Promise.all([...files].map(fileAsDataUrl));setForm(x=>({...x,reference_images:[...(x.reference_images||[]),...images]}));};const saveRoom=async e=>{e.preventDefault();const item={...form,id:editing?.id,photo:form.reference_images[0]||form.photo};await offlineWrite(editing?'PUT':'POST',editing?`/api/home/locations/${editing.id}`:'/api/home/locations',item);setForm(blank);setEditing(null);load();};const saveEdge=async e=>{e.preventDefault();await offlineWrite('POST','/api/home/connections',edge);setEdge({from_location_id:'',to_location_id:'',instruction:''});load();};return <div className="space-y-6"><h2 className="font-heading font-bold text-2xl">Home Camera Map</h2><form onSubmit={saveRoom} className="bg-white p-6 rounded-3xl border space-y-3"><h3 className="font-bold">{editing?'Edit room photos':'Add room photos'}</h3><input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Room name" className="w-full p-3 border rounded-xl"/><input value={form.familiar_name} onChange={e=>setForm({...form,familiar_name:e.target.value})} placeholder="Familiar name" className="w-full p-3 border rounded-xl"/><textarea value={form.voice_description} onChange={e=>setForm({...form,voice_description:e.target.value})} placeholder="Voice description" className="w-full p-3 border rounded-xl"/><input type="file" accept="image/*" multiple onChange={e=>photos(e.target.files)}/><p>{form.reference_images.length} corner photo(s) selected</p><button className="px-4 py-2 rounded-xl bg-sage-600 text-white">Save room</button></form><form onSubmit={saveEdge} className="bg-white p-6 rounded-3xl border space-y-3"><h3 className="font-bold">Add route connection</h3><select required value={edge.from_location_id} onChange={e=>setEdge({...edge,from_location_id:e.target.value})} className="w-full p-3 border rounded-xl"><option value="">From</option>{rooms.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select><select required value={edge.to_location_id} onChange={e=>setEdge({...edge,to_location_id:e.target.value})} className="w-full p-3 border rounded-xl"><option value="">To</option>{rooms.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select><input required value={edge.instruction} onChange={e=>setEdge({...edge,instruction:e.target.value})} placeholder="Walk through the hall, then turn left" className="w-full p-3 border rounded-xl"/><button className="px-4 py-2 rounded-xl bg-sage-600 text-white">Save route</button></form><HomeMapDiagram locations={rooms} connections={edges}/><div className="grid md:grid-cols-2 gap-3">{rooms.map(r=><div key={r.id} className="bg-white border rounded-xl p-4"><b>{r.familiar_name||r.name}</b><p>{(r.reference_images||[]).length} corner photos</p><button onClick={()=>{setEditing(r);setForm({...blank,...r,reference_images:r.reference_images||[r.photo].filter(Boolean)})}}>Edit</button></div>)}</div></div>;}
class AppErrorBoundary extends React.Component { constructor(props){super(props);this.state={error:null};} static getDerivedStateFromError(error){return {error};} componentDidCatch(error,info){console.error('SmritiSaathi render failure',error,info);} render(){if(this.state.error)return <pre style={{whiteSpace:'pre-wrap',padding:20,color:'#991b1b',background:'#fff1f2'}}>SmritiSaathi could not start:{'\n'}{this.state.error.message}{'\n'}{this.state.error.stack}</pre>;return this.props.children;} }
...
root.render(<AppErrorBoundary><CogniCareApp /></AppErrorBoundary>);
